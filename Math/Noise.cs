...
namespace Glacier.Common.GlacerMath
{
    public class NoiseGenerator
    {
        public class NoiseParams
        {
            public int NOISE_MAGIC_X;
            public int NOISE_MAGIC_Y;
            public int NOISE_MAGIC_Z;
            public int NOISE_MAGIC_SEED;
        ...
        /// <summary>
        /// The parameters to use for generating this world.
        /// </summary>
        public NoiseParams WorldGenConst = new NoiseParams();
        double persistence;
        double frequency;
        double amplitude;
        int octaves;
        ...
        /// <summary>
        /// Creates a new NoiseGenerator using the provided tuning values.
        /// </summary>
        ...
        public NoiseGenerator(double persistence, double frequency, double amplitude, int octaves, int randomSeed)
        {
        ...
        public async Task FillMap2D(double[,] map, Point Offset, int seamWidthX, int seamWidthY)
        {
            ...
            await Task.Factory.StartNew(() =>
            {
                for (int x = 0; x < mapWidth; x++)
                {
                    ...
                    {
                        map[x, y] = Get2D(x + Offset.X, y + Offset.Y);
                    }
                }
                //FixHorizontalSeams(ref map, seamWidthX);
                //FixVerticalSeams(ref map, seamWidthY);
            });            
        }
        ...
        /// INTERNAL \\\
        /// ******** \\\
        public static void FixHorizontalSeams(ref double[,] source, int seamWidth)
        {
            int mapWidth = source.GetUpperBound(0) + 1;
            int mapHeight = source.GetUpperBound(1) + 1;
            for (int x = 0; x < seamWidth; x++)
            {
                double dest = x / (double)seamWidth;
                for (int y = 0; y < mapHeight; y++)
                {
                    int src = seamWidth - seamWidth + x;
                    source[src, y] = MathHelper.Lerp((float)source[src, y], (float)source[seamWidth - x, y], (float)dest);
                }
            }
        }
        public static void FixVerticalSeams(ref double[,] source, int seamHeight)
        {
                ...
                for (int x = 0; x < width; x++)
                {
                    ...
                    for (int o = x - k; o < x + k; o++)
                    {
                        if (o < 0 || o >= width)
                            continue;
                        s1 += source[o, y];
                        s2 += source[o, height - y - 1];
                        c++;
                    }
                    source[x, y] = s1 / (float)c;
                    source[x, height - y - 1] = s2 / (float)c;
                }
            }
        }
